...
This makes it such that it is trivial to handle complex configurations, such as [[file:emacs.nix::python = pkgs.python3.override {][pylsp with plugins]] or [[file:emacs.nix::(treesit-grammars.with-grammars (g: with g; \[][treesitter grammars installation process]] (which usually envolves some stateful installations outside of emacs).
...
It also relies on [[file:emacs.nix::packageOverrides = self: super: {][python lsp server with builtin ruff support]]
#+begin_src emacs-lisp :tangle yes
